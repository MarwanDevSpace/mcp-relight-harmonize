...
import { readImage, RawImage } from "./image_io";

export function calculateCctFromRgb(rMean: number, gMean: number, bMean: number): number {
...
export function computeSurfaceNormalsAndLightVector(
  lum: Float32Array,
  width: number,
  height: number
): { normalField: Float32Array; roughness: number; lightVector: [number, number, number]; angles: LightingAngle } {
  ...
  let lumSum = 0;
  for (let i = 0; i < lum.length; i++) lumSum += lum[i];
  ...
  let lightVec: [number, number, number];
  if (weightSum < 1e-5) {
...
export function analyzeOpticalProfileImpl(imagePath: string): OpticalProfileReport {
  const resolvedPath = validateImagePath(imagePath);
  const raw = readImage(resolvedPath);
  ...
  const lumValues: number[] = new Array(numPixels);
  ...
  for (let i = 0; i < numPixels; i++) {
    ...
    if (lum > 235) specularCount++;
    if (lum < 25) shadowsCount++;
  }
  ...
  const warmth =
    cctKelvin < 4000 ? "warm tungsten" : cctKelvin < 6500 ? "neutral daylight" : "cool atmospheric";
  const summary =
    `${width}x${height} image with ${warmth} illumination (${Math.round(cctKelvin)}K), ` +
