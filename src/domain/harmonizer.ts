...
import { calculateCctFromRgb } from "./optical_analyzer";
...
function labToRgb(l: number, alpha: number, beta: number): [number, number, number] {
...
export function reinhardColorTransfer(fg: RawImage, bg: RawImage): RawImage {
  const out = cloneImage(fg);
...
export function harmonizeCompositeImpl(
  foregroundPath: string,
  backgroundPath: string,
  blendMode = "seamless",
  outputPath?: string
): HarmonizeCompositeResult {
  const fgResolved = validateImagePath(foregroundPath);
  const bgResolved = validateImagePath(backgroundPath);

  const fgRaw = readImage(fgResolved);
  const bgRaw = readImage(bgResolved);
  ...
  const bgCct = calculateCctFromRgb(sumR / bgTotal, sumG / bgTotal, sumB / bgTotal);

  // 3. Composite onto background
  const composite = cloneImage(bgRaw);
  ...
  for (let y = 0; y < fgRaw.height; y++) {
    const bgY = topY + y;
    if (bgY < 0 || bgY >= bgRaw.height) continue;

    for (let x = 0; x < fgRaw.width; x++) {
      const bgX = leftX + x;
      if (bgX < 0 || bgX >= bgRaw.width) continue;
      ...
      if (alpha > 0) {
        composite.data[bgIdx] = Math.round(
          harmonizedFg.data[fgIdx] * alpha + composite.data[bgIdx] * (1.0 - alpha)
        );
        composite.data[bgIdx + 1] = Math.round(
          harmonizedFg.data[fgIdx + 1] * alpha + composite.data[bgIdx + 1] * (1.0 - alpha)
        );
        composite.data[bgIdx + 2] = Math.round(
          harmonizedFg.data[fgIdx + 2] * alpha + composite.data[bgIdx + 2] * (1.0 - alpha)
        );
      }
    }
  }
  ...
  const outPath =
    ...
    path.join(
      outDir,
      `harmonized_${path.basename(fgResolved, path.extname(fgResolved))}_${path.basename(
        bgResolved,
        path.extname(bgResolved)
      )}.png`
    );

  writeImage(outPath, composite);
