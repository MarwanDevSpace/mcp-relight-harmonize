...
export function buildDynamicDiagnostics(
  ...
  const nanoPrompt =
    `optics relight, cct ${Math.round(cctKelvin)}K, light vector azimuth ${Math.round(azimuthDeg)} deg elevation ${Math.round(elevationDeg)} deg, ` +
...
export function extractLayersImpl(
  ...
  const { roughness, lightVector, angles } = computeSurfaceNormalsAndLightVector(lumArray, width, height);
  ...
  writeImage(hlPath, { width, height, data: bufHighlights });
  writeImage(shPath, { width, height, data: bufShadows });
  writeImage(aoPath, { width, height, data: bufAO });
  writeImage(edPath, { width, height, data: bufEdges });
  writeImage(normPath, { width, height, data: bufNormals });
  writeImage(chrPath, { width, height, data: bufChroma });
