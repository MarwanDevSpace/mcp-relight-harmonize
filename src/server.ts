...
import { analyzeOpticalProfileTool } from "./tools/analyze_optical";
import { generateRelightVariationsTool } from "./tools/generate_relight";
import { harmonizeCompositeTool } from "./tools/harmonize";
import { synthesizeDiffusionPromptTool } from "./tools/synthesize_prompt";
import { listCachedVariationsTool } from "./tools/list_cache";
import { getPresetsJson } from "./resources/presets";
      ...
      tools: [
        {
          ...
          description:
            "Extract physical optical metrics from an image, including Correlated Color Temperature (CCT in Kelvin), " +
          ...
          inputSchema: {
            ...
            properties: {
              image_path: {
                type: "string",
                description:
                  "Absolute or workspace-relative path to a local image file (.png, .jpg, or .jpeg). " +
                  "Must be an existing image under 50 MB.",
              },
              extract_layers: {
                type: "boolean",
                description:
                  "If true, decomposes image into 6 analytical layers (Highlights, Shadows, Ambient Occlusion, Edges, Depth Normals, Chroma/Saturation) saved to Layers/ directory and generates Layer.md.",
              },
              layers_dir: {
                type: "string",
                description:
                  "Optional target directory to store the 6 analytical layer images and Layer.md (defaults to 'Layers').",
              },
              user_intent: {
                type: "string",
                description:
                  "Optional creative or corrective intent to dynamically tailor layer diagnostics, recommendations, and diffusion prompts.",
              },
            },
            required: ["image_path"],
          },
        ...
        },
        {
          ...
          description:
            "Generate 4 physically-grounded relit image variations on disk (Ambient fill, Dramatic chiaroscuro, " +
          ...
          inputSchema: {
            ...
            properties: {
              ...
              target_lighting: {
                ...
                default: "All",
                description:
                  "Lighting preset selection: 'Ambient' (+0.8 EV lifted shadows, 5500K daylight), " +
        ...
        },
        {
          ...
          description:
            "Harmonize and composite a foreground subject cutout onto a background environment scene image using Reinhard " +
          ...
          inputSchema: {
            ...
            properties: {
              ...
              blend_mode: {
                ...
                default: "seamless",
                description:
                  "Blending mode: 'seamless' (applies Reinhard color transfer + contact shadow + alpha blend) " +
        ...
        },
        {
          ...
          description:
            "Synthesize precision enhancement and relighting diffusion prompts based on physical optical analysis of an image, " +
          ...
          inputSchema: {
            ...
            properties: {
              ...
              target_model: {
                ...
                default: "universal",
                description:
                  "Target generative engine format: 'universal' (outputs natural descriptive studio directives with 85mm prime lens and physical illumination compatible with Any Image Generator) " +
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
      case "analyze_optical_profile": {
        ...
        envelope = analyzeOpticalProfileTool(imagePath, extractLayers, layersDir, userIntent);
        break;
      }
      case "generate_relight_variations": {
        ...
        envelope = generateRelightVariationsTool(imagePath, targetLighting, outputDir);
        break;
      }
      case "harmonize_composite": {
        ...
        envelope = harmonizeCompositeTool(fgPath, bgPath, blendMode);
        break;
      }
      case "synthesize_diffusion_prompt": {
        ...
        envelope = synthesizeDiffusionPromptTool(imagePath, userIntent, targetModel);
        break;
      }
      case "list_cached_variations": {
        const cacheDir = args.cache_dir ? String(args.cache_dir) : undefined;
        envelope = listCachedVariationsTool(cacheDir);
        break;
      }
      default:
